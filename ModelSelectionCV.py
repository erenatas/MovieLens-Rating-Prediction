...
def model_selection_cv():
    ...
    scores = model_selection.cross_val_score(regrOLS, X_train, y_train, scoring='neg_mean_squared_error', cv=5)
    ...
    for l in lambdas:
        ...
        scores = model_selection.cross_val_score(regrRidge, X_train, y_train, scoring='neg_mean_squared_error', cv=5)
        end = time.time()
        t = end - start
        ...
        scores = model_selection.cross_val_score(regrLasso, X_train, y_train, scoring='neg_mean_squared_error', cv=5)
    ...
    for l in lambdas:
        ...
        for l1R in l1Ratios:
            ...
            scores = model_selection.cross_val_score(regrElasNet, X_train, y_train, scoring='neg_mean_squared_error',
                                                     cv=5)
            end = time.time()
            t = end - start
    ...
    for c in C:
        ...
        for e in eps:
            ...
            scores = model_selection.cross_val_score(svr, X_train, y_train, scoring='neg_mean_squared_error', cv=5)
            end = time.time()
            t = end - start
    ...
    for n in numTrees:
        for mf in maxFeatures:
            for d in maxDepth:
                ...
                scores = model_selection.cross_val_score(randFor, X_train, y_train, scoring='neg_mean_squared_error',
                                                         cv=5)
                end = time.time()
                t = end - start
    ...
    for l in lossType:
        for n in numTrees:
            for d in maxDepth:
                ...
                scores = model_selection.cross_val_score(boosting, X_train, y_train, scoring='neg_mean_squared_error',
                                                         cv=5, n_jobs=1)
                end = time.time()
                t = end - start
                print('Regressor using boosting: loss type = %s, Least CV error: %.2f and time : %.3f' % (
                    l, np.min(-scores), t))
            print('\n')
